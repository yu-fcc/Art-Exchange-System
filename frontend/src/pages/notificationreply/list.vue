            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('notificationreply/add')">
                                <router-link :to="`/notificationreply/add`">
                                    <Button :label="$t('addNewNotificationReply')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/notificationreply'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="reply_id" :header="$t('replyId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/notificationreply/view/${data.reply_id}`">
                                                    {{ data.reply_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="notif_id" :header="$t('notifId')" >
                                            <template #body="{ data, index }">
                                                {{ data.notif_id }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'notificationreplylist',
		},
		apiPath: {
			type: String,
			default: 'notificationreply/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/notificationreply/view/${data.reply_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('notificationreply/view')
		},
		{
			label: () => $t('edit'),
			to: `/notificationreply/edit/${data.reply_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('notificationreply/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.reply_id) },
			icon: "pi pi-trash",
			visible: auth.canView('notificationreply/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('notificationReply');

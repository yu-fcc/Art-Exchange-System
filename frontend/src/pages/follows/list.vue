                        ...
                        <div >
                            ...
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="follow_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="follow_id" :header="$t('followId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/follows/view/${data.follow_id}`">
                                                    {{ data.follow_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="followed_id" :header="$t('followedId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('users')" v-if="data.followed_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.followed_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/follows/view/${data.follow_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('follows/view')
		},
		{
			label: () => $t('edit'),
			to: `/follows/edit/${data.follow_id}`,
			icon: "pi pi-pencil",

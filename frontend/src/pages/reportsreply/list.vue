            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('reportsreply/add')">
                                <router-link :to="`/reportsreply/add`">
                                    <Button :label="$t('addNewReportsReply')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/reportsreply'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="reply_id" :header="$t('replyId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/reportsreply/view/${data.reply_id}`">
                                                    {{ data.reply_id }}
                                                </router-link>
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'reportsreplylist',
		},
		apiPath: {
			type: String,
			default: 'reportsreply/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/reportsreply/view/${data.reply_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('reportsreply/view')
		},
		{
			label: () => $t('edit'),
			to: `/reportsreply/edit/${data.reply_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('reportsreply/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.reply_id) },
			icon: "pi pi-trash",
			visible: auth.canView('reportsreply/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('reportsReply');
